      ...
      <div className="max-w-7xl mx-auto px-6 py-16 grid grid-cols-1 md:grid-cols-4 gap-10">
            ...
            {socialLinks.map((item) => {
              ...
              return (
                <a
                  ...
                  className="w-9 h-9 border border-[#f5f0e8]/15 rounded-full flex items-center justify-center text-[#f5f0e8]/40 hover:border-[#c9a84c] hover:text-[#c9a84c] transition-all duration-200"
                >
        ...
        <div>
          ...
          <ul className="space-y-3">
            {destinations.map((item) => (
              <li key={item}>
                <Link href="/destinations" className="text-sm text-[#f5f0e8]/55 hover:text-[#c9a84c] transition-colors">
                  {item}
                </Link>
              </li>
            ))}
        ...
        <div>
          ...
          <ul className="space-y-3">
            {tours.map((item) => (
              <li key={item}>
                <Link href="/packages" className="text-sm text-[#f5f0e8]/55 hover:text-[#c9a84c] transition-colors">
                  {item}
                </Link>
              </li>
            ))}
        ...
        <div>
          ...
          <ul className="space-y-3">
            {companyLinks.map((item) => (
              <li key={item.label}>
                <Link href={item.href} className="text-sm text-[#f5f0e8]/55 hover:text-[#c9a84c] transition-colors">
                  {item.label}
                </Link>
              </li>
            ))}
      ...
      <div className="border-t border-white/10 px-6 py-5">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-3">
          <p className="text-xs text-[#f5f0e8]/25">
            2025 K2 Trek and Tours. PTDC Licensed. All rights reserved.
          </p>
          <div className="flex gap-6">
            {['Privacy Policy', 'Terms', 'Sitemap'].map((item) => (
              <Link key={item} href="#" className="text-xs text-[#f5f0e8]/25 hover:text-[#c9a84c] transition-colors">
                {item}
              </Link>
            ))}
          </div>
